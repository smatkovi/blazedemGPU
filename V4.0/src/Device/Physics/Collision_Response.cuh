		                                                ...
		                                                uint P_type  )
{
   ...
   float  Fnormal    =  ParticleObject[P_type].surface_Kn*pen_dis;
   float3 Fdiss =  (ParticleObject[P_type].surface_Cn)*Vnormal;
   ...
   else
   {
    ...
    if( mag_Vtangent > 0.0f )
    {
		...
		float tan_f = ( 0.050f*(ParticleObject[P_type].mass/SimParms.InitalDelta_t) )*((ParticleObject[P_type].surface_Fric_static*(mag_Vtangent/Velf)));
															 ...
															 float3 Vel_Surface )
{
	 ...
	 if( mag_Vt_Rel > 0.0f )
     {
		...
		Tangential_Force = -1.0f*tan_f*(Vt_Rel/mag_Vt_Rel)*( ParticleObject[P_typeA].mass/SimParms.InitalDelta_t)*0.050f;
														...
														)
{
     ...
     if(SimParms.force_model==nonlinear)
     {
    	 Fnormal =   ParticleObject[P_typeA].PP[P_typeB].Kn*(pen_dis*sqrt(pen_dis));
     }
     else
     {
    	 Fnormal =   ParticleObject[P_typeA].PP[P_typeB].Kn*pen_dis;
     }
     ...
     /* Get Relative Tangential Velocity */
     float3  Vt_Rel    = ((Vel_A + cross(VelA_Ang,R_Body_COMA))- (dot( (Vel_A + cross(VelA_Ang,R_Body_COMA)),normal)*normal)) - ((Vel_B + cross(VelB_Ang,R_Body_COMB)) - (dot( (Vel_B + cross(VelB_Ang,R_Body_COMB)),normal)*normal));
	 ...
	 if( mag_Vt_Rel > 0.0f )
     {
		...
		/* Add energy dissipated */
		if (SimParms.EnergyCalc  && tally==1)
				{
	   ...
	   /* 3. Rotational Force */
		...
		if ( SimParms.Attraction >0.0f)
		{
			 Force.trans -= (SimParms.Attraction*rpos*MassA)/SimParms.InitalDelta_t;
		}
		                                                 ...
		                                                 uint P_type        )
{
      ...
      float3 Fdiss = ParticleObject[P_type].surface_Cn*Vn;
      float  Fn    =(ParticleObject[P_type].surface_Kn*pen_dis);
