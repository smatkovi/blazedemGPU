...
struct SimulationInfo
{
  ...
  float Vel_Limit;
  float VolumeObject_CylinderBoundR;
  ...
  float valid_zone_X_Start;
  float valid_zone_X_End;
  float valid_zone_Y_Start;
  float valid_zone_Y_End;
  float valid_zone_Z_Start;
  float valid_zone_Z_End;
...
struct InitConfig
{
   ...
   bool  get_wall_points;
   bool  get_vobject_points;
   bool  get_particle_points;
...
struct OpenGLOBJ
{
  ...
  int   surface_tally_format;
  int   surface_tally_forces;
  ...
  float3 colors[32];
  bool manual_layercol;
  int num_layers;
  int num_perLayer[32];
  int particle_type_perLayer[32];
...
struct DOBJ
{
  ...
  float3   vertex       [10]; /* Rotated on CPU */
  int      num_faces;         /* Number of planar surfaces */
  Surface  faces     [4];     /* Normal, Centroid etc */
  int       num_edges;
  Edge_Def  edge[8];

  /* Animation properties */
  bool is_attached_DWorld; /* Will rotate, translate with the world */
  
  bool is_translating;
  bool is_rotating;

  float3 translate_vel;
  float3 rotate_vel;
