...
public:
	...
	void IDevice_DEM_UpdateSimMulti()         { Device_DEM_UpdateSimMulti() ;};
	...
	void IDevice_Get_System_State( float3 *h_pos,Quaterion *h_Ornt, float3 *h_vel,float3 *h_Avel,

	                               uint *h_type, int *h_pid  , int *Number_Particles                 )
	{Device_Get_System_State( h_pos,h_Ornt, h_vel, h_Avel,

	h_type, h_pid,Number_Particles);};


	void IDevice_Get_Positions ( float3 *h_pos, Quaterion *h_Quart_ornt, uint *h_type,
			                     int *P_ID, int *Number_Particles )
	{ Device_Get_P_Positions(h_pos,h_Quart_ornt,h_type,P_ID, Number_Particles) ;};

	void IDevice_Get_Particle_Velocity(float3 *H_Vel, float3 *H_Rvel, int flag,uint *h_type,
            int *P_ID, int *Number_Particles)
	{Device_Get_P_Velocity(H_Vel,H_Rvel,flag,h_type,P_ID,Number_Particles);};

   void IDevice_Get_ID ( uint *h_type,
			                     int *P_ID, int *Number_Particles )
								 { Device_Get_PID(h_type,P_ID, Number_Particles) ;};



	void IDevice_Get_W_Forces(float3 *WallForces,float3 *WallContact_Point){Device_Get_W_Forces(WallForces,WallContact_Point);};

	void IDevice_Get_P_Forces(uint *PnumContacts,float3 *PContact_Point){Device_Get_P_Forces(PnumContacts,PContact_Point);};

	void IDevice_Get_V_Forces(float3 *VObjectForces,float3 *VObjectContact_Point){Device_Get_V_Forces(VObjectForces,VObjectContact_Point);};
