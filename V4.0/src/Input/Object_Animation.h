...
/*---------------------------------------------------------------------------*/
/*        Rotates a Volume Object Incrementally (Variable Speed)          */
/*---------------------------------------------------------------------------*/
void Rotate_VolObject_Inc(float3 about)
{ 


  for( int index=0;index<m_num_KVolumeObjects;index++ )
  {

	if(m_KVolumeObject[index].is_attached_DWorld )
	{
	 float3 sum = make_float3(0.0f,0.0f,0.0f);

     /* Rotate Vertex */
     for(int k=0;k<m_KVolumeObject[index].num_vertex;k++)
     {
    	 float3 point = m_KVolumeObject[index].vertex[k] - about;

    	 m_KVolumeObject[index].vertex[k].x = cos(m_Mill_SimData.RadPerStep)*point.x + sin(m_Mill_SimData.RadPerStep)*point.y;
    	 m_KVolumeObject[index].vertex[k].y = -sin(m_Mill_SimData.RadPerStep)*point.x + cos(m_Mill_SimData.RadPerStep)*point.y;
    	 m_KVolumeObject[index].vertex[k] = m_KVolumeObject[index].vertex[k] + about;

     }

    // printf("%f\n",m_KDynamicObject[index].vertex[0].x);
     /* Update Surfaces */
     for(int j=0;j<m_KVolumeObject[index].num_faces;j++)
     {
    	 sum = make_float3(0.0f,0.0f,0.0f);

         for(int f=0;f<m_KVolumeObject[index].faces[j].num_vertex;f++)
         {
	       sum = sum + m_KVolumeObject[index].vertex[ m_KVolumeObject[index].faces[j].vertex_Order[f] ];
         }

         m_KVolumeObject[index].faces[j].centroid = (1.0f/m_KVolumeObject[index].faces[j].num_vertex)*sum;

         float3  V1 = m_KVolumeObject[index].vertex[m_KVolumeObject[index].faces[j].vertex_Order[1]] -
        		 m_KVolumeObject[index].vertex[m_KVolumeObject[index].faces[j].vertex_Order[0]];

         float3  V2 = m_KVolumeObject[index].vertex[m_KVolumeObject[index].faces[j].vertex_Order[2]] -
        		 m_KVolumeObject[index].vertex[m_KVolumeObject[index].faces[j].vertex_Order[1]];


         float3  V3 = m_KVolumeObject[index].vertex[m_KVolumeObject[index].faces[j].vertex_Order[3]] -
        		 m_KVolumeObject[index].vertex[m_KVolumeObject[index].faces[j].vertex_Order[2]];


         float3  V4 = m_KVolumeObject[index].vertex[m_KVolumeObject[index].faces[j].vertex_Order[0]] -
        		 m_KVolumeObject[index].vertex[m_KVolumeObject[index].faces[j].vertex_Order[3]];


         float3 n         = cross(V2,V1);

         m_KVolumeObject[index].faces[j].normal = n/length(n);

         m_KVolumeObject[index].faces[j].area= 0.500000f*length(n) + 0.500000f*length(cross(V3,V4));

     }


     /* Rotate COM */
     float3 point = m_KVolumeObject[index].COM - about;
     m_KVolumeObject[index].COM.x = cos(m_Mill_SimData.RadPerStep)*point.x + sin(m_Mill_SimData.RadPerStep)*point.y;
     m_KVolumeObject[index].COM.y = -sin(m_Mill_SimData.RadPerStep)*point.x + cos(m_Mill_SimData.RadPerStep)*point.y;

     m_KVolumeObject[index].COM   = m_KVolumeObject[index].COM + about;
...
/*---------------------------------------------------------------------------*/
/*        Rotates a Volume Object from start position fixed speed         */
/*---------------------------------------------------------------------------*/
  ...
  for( int index=0;index<m_num_KVolumeObjects;index++ )
  {
	...
	if(m_KVolumeObject[index].is_attached_DWorld )
    {
     ...
     for(int k=0;k<m_KVolumeObject[index].num_vertex;k++)
     {
    	 ...
    	 m_KVolumeObject[index].vertex[k].x = cos(Rotation_Angle)*point.x + sin(Rotation_Angle)*point.y;
    	 m_KVolumeObject[index].vertex[k].y = -sin(Rotation_Angle)*point.x + cos(Rotation_Angle)*point.y;
    	 m_KVolumeObject[index].vertex[k] += about;
     ...
     /* Rotate COM */
     float3 point = m_KVolumeObject[index].COM - about;
     m_KVolumeObject[index].COM.x = cos(Rotation_Angle)*point.x + sin(Rotation_Angle)*point.y;
     m_KVolumeObject[index].COM.y = -sin(Rotation_Angle)*point.x + cos(Rotation_Angle)*point.y;
	...
	if( m_KWorldObject[index].is_rotating )
	{
      ...
      for(int k=0;k<m_KWorldObject[index].num_vertex;k++)
      {
       ...
       for(int j=0;j<m_KWorldObject[index].num_surfaces;j++)
       {
         ...
         {
        	 m_KWorldObject[index].surfaces[j].area= 0.50f*length(n);
         }
...
/*---------------------------------------------------------------------------*/
/*         Rotates a World Object incrementally fixed speed                */
/*---------------------------------------------------------------------------*/
...
void Translate_VolObject(int index, float3 Disp)
{
	for(int k=0;k<m_KVolumeObject[index].num_vertex;k++)
	{
       m_KVolumeObject[index].vertex[k]+= Disp;
	}
	...
	float3 sum = make_float3(0.0f,0.0f,0.0f);
     ...
     /* Rotate COM */
     m_KVolumeObject[index].COM   = m_KVolumeObject[index].COM + Disp;
