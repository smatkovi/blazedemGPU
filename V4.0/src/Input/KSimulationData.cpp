...
KSimulationData::KSimulationData( string ProjectFolder, string WorldName )
{
  ...
  LogFile<<endl;
  LogFile<<"   ********************************************************************"<<endl;
  LogFile<<"   *                  Welcome to BLAZE-DEM V4.0                       *"<<endl;
  LogFile<<"   *                          MultiGPU                                *"<<endl;
  LogFile<<"   *                Build: (1/2016) CUDA 7.5                          *"<<endl;
  ...
  if(!WorldF.is_open())
  {
	  cout<<"!Error-INPUT: Simulation File file not found!"<<endl;
	  exit(1);
  }
  else
  {
	...
	/* Will activate print flags in Kernels */
	...
	else if(m_Sim_type==Pulp_Lift)
    {
		  LogFile<<"INFO-INPUT: Pulp_Lift Simulation mode "<<endl;
	}
	else
	{
	...
	for (int i=0;i<m_num_WorldObjects;i++)
	{
	  ...
	  m_WorldObjectsList[i].is_translating = false;
	   ...
	   {
		 cout<<" Translating world object "<<endl;
		 m_WorldObjectsList[i].is_translating = true;	
		 WorldF>>dumS>>m_WorldObjectsList[i].translate_vel.x>>m_WorldObjectsList[i].translate_vel.y>>m_WorldObjectsList[i].translate_vel.z;
	   }
	   else
	   {
		 m_WorldObjectsList[i].is_rotating = false;
	...
	else
	{
	  ...
	  exit(1);
		...
		for (int i=0;i<m_num_ParticleObjects;i++)
		{
	       ...
	       cout<<"INFO-INPUT: Opening Object "<<filepath<<" "<<i+1<<" of "
		   ...
		   {
			 cout<<"!Error-INPUT: PObject Not found!"<<endl;
			 exit(1);
		   }
		   else
		   {
		...
		cout<<"INFO-INPUT: Reading particle objects: Done "<<endl;
        	...
        	else/* Manual mode*/
        	{
				...
				for (int i=0;i<m_num_DynamicObjects;i++)
				{
				   ...
				   WorldF>>dumI>>DYOBJ_Fname;
				     ...
				     if(dumI==99) /* Static Object */
				     {
					   m_DynamicObjectsList[i].is_attached_DWorld = false;
					   m_DynamicObjectsList[i].is_translating     = false;
					   m_DynamicObjectsList[i].is_rotating        = false;
					   cout<<"Volume Object is static "<<endl;
				     }
				     else if(dumI==98)/* 98 translating */
				     {
				       m_DynamicObjectsList[i].is_attached_DWorld = false;
					   m_DynamicObjectsList[i].is_rotating        = false;
				 	   m_DynamicObjectsList[i].is_translating     = true;
					   WorldF>>dumS>>m_DynamicObjectsList[i].translate_vel.x>>m_DynamicObjectsList[i].translate_vel.y>>m_DynamicObjectsList[i].translate_vel.z;
				       cout<<"Volume Object is translating "<<endl;
				 	 }
					 else if(dumI==97)/* 97 rotating */
				     {
				       m_DynamicObjectsList[i].is_attached_DWorld = false;
				 	   m_DynamicObjectsList[i].is_translating     = false;
					   m_DynamicObjectsList[i].is_rotating        = true;
					   WorldF>>dumS>>m_DynamicObjectsList[i].rotate_vel.x>>m_DynamicObjectsList[i].rotate_vel.y>>
						   m_DynamicObjectsList[i].rotate_vel.z;

				       cout<<"Volume Object is rotating "<<endl;
				 	 }
				     else
				     {
...
//		}
		...
		else
		{
		  ...
		  for (int i=0;i<3;i++)
		  {
			  WorldF>>Kill_zone[i].x>>Kill_zone[i].y;
		  }
		}
		...
		if(dumI==-1)
		{
			m_OpenGLObject.write_vtk = true;
			WorldF>>m_OpenGLObject.vtk_steps;
		}
		else
		{
         m_OpenGLObject.write_vtk = false;
		}
		...
		if(dumI==0)
		{
		   m_OpenGLObject.display_cut = false;
		}
		else
		{
          m_OpenGLObject.display_cut = true;
		}

		if (m_OpenGLObject.display_cut)
		{
			WorldF>>m_OpenGLObject.Z_cut_start>>m_OpenGLObject.Z_cut_end;
		}
		...
		if(dumI==1)
		{
		  ...
		  WorldF>>m_OpenGLObject.surface_tally_calc_steps>>
			  m_OpenGLObject.surface_tally_format>>
			  m_OpenGLObject.surface_tally_forces;
		}
		else
		{
		...
		else if(m_Sim_type==silo)
		{
          ...
          else
          {
			  ...
			  WorldF>>m_siloObject.flow_steps;
          }
		...
		m_OpenGLObject.custom_color = false;
		m_OpenGLObject.manual_layercol = false;
		if(m_Sim_type==silo)
		{
		    WorldF>>dumS;
			WorldF>>dumS;
			...
			if(dumI==1)
			{
			  m_OpenGLObject.custom_color = true;
			}
			if(m_OpenGLObject.custom_color)
			{
				WorldF>>dumS>>m_OpenGLObject.num_colors;
				for (int i=0;i<m_OpenGLObject.num_colors;i++)
				{
					WorldF>>dumS>>m_OpenGLObject.colors[i].x>>m_OpenGLObject.colors[i].y>>m_OpenGLObject.colors[i].z;
				}
			}
			...
			if(dumI==1)
			{
				m_OpenGLObject.manual_layercol = true;
			}
			
			if(m_OpenGLObject.manual_layercol)
			{
				WorldF>>dumS>>m_OpenGLObject.num_layers;
				printf(" Reading %d Layers \n",m_OpenGLObject.num_layers);
				for (int i=0;i<m_OpenGLObject.num_layers;i++)
				{
					WorldF>>m_OpenGLObject.num_perLayer[i]>>m_OpenGLObject.particle_type_perLayer[i];
				}
			}
...
void KSimulationData::ReadWObject(int i)
{
  ...
  int    dumI;
			...
			for( int j=0; j<m_WorldObjectsList[i].num_surfaces;j++ )
			{
				...
				{
					m_WorldObjectsList[i].surface_type= cone_trunc;
				}
				else
				{
				...
				if( m_WorldObjectsList[i].surface_type == plane )
				{
				 ...
				 else
				 {
					 ...
					 if(m_WorldObjectsList[i].surface_type==cone_trunc)
					 {
						 printf("reading cone\n");
						 WObjectsF>>dumS>>m_WorldObjectsList[i].cyl_geo.radius_bot;
						 m_WorldObjectsList[i].cyl_geo.radius_bot*=0.50f;
				    ...
				    cout<<"Cylinder Axis:"<<m_WorldObjectsList[i].cyl_geo.AXIS.x<<
				...
				if( m_WorldObjectsList[i].surface_type == plane )
				{
				   ...
				   if( m_WorldObjectsList[i].surfaces[j].num_vertex==3 )
				   {
		             ...
		             m_WorldObjectsList[i].surfaces[j].area = 0.50f*length(n);
			        ...
			        if( m_WorldObjectsList[i].surfaces[j].num_vertex==4 )
			        {
			           ...
			           float3  V3 = m_WorldObjectsList[i].vertex[m_WorldObjectsList[i].surfaces[j].vertex_Order[3]] -
					    ...
					    float3 n         = cross(V2,V1);
					    m_WorldObjectsList[i].surfaces[j].normal = n/length(n);
					    m_WorldObjectsList[i].surfaces[j].area = 0.5f*length(n) + 0.5f*length(cross(V4,V3)) ;

						if ( length(m_WorldObjectsList[i].surfaces[j].normal - (cross(V4,V3)/length(cross(V4,V3))))>0.0010f )
						{
                          printf(" Warning Plane %d not co-planar use triangles ",j);
						  PrintFloat3(m_WorldObjectsList[i].surfaces[j].normal);
						  PrintFloat3( (cross(V4,V3)/length(cross(V4,V3))));
...
void KSimulationData::ReadPObject_Sphere(int i)
{
    ...
    PObjectsF>>dumS;
    	...
    	m_ParticleObjectsList[i].volume = ((4.0/3.0)*2.0*asin(1.0)*pow(m_ParticleObjectsList[i].radius,3.0) );

    	//m_ParticleObjectsList[i].mass =1.0f;

        printf("Volume(cm3) %f  mass(kg) %f \n",m_ParticleObjectsList[i].volume, m_ParticleObjectsList[i].mass);
        ...
        if(m_ForceSpec==0)
        {
		    ...
		    if(m_WorldTitle=="FILL")
	        {
              printf("!Warning: Setting COR 0.30 for filling \n");
              COR[i] = 0.30f;
			  COR_S  = 0.30f;
			  COR_D  = 0.30f;
			}
...
void KSimulationData::ReadVObject(int i)
{
    string dumS;
    int    dumI;
