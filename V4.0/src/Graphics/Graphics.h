...
#include "../Input/Object_Animation.h"
...
void SetColor_Ramp()
{
	...
	if(m_OpenGL.custom_color)
	{
		for (int i=0;i<m_OpenGL.num_colors ;i++)
		{
           
			Col[i][0]=m_OpenGL.colors[i].x;
			Col[i][1]=m_OpenGL.colors[i].y;
			Col[i][2]=m_OpenGL.colors[i].z;
...
void ReshapeGL(int w, int h)
{
  ...
  glLoadIdentity();
	...
	{
	 x = 2.0f*m_KWorldObject[0].cyl_geo.radius*0.70f;
	 glOrtho(-x, x, -x/aspect, x/aspect, -x, x);
	}
	else
	{
		x= max(max(TotalSize.x,TotalSize.y),TotalSize.z);
		glOrtho(-0.25*x, x*0.25, -0.25*x, x*1.25, -x, x*1.25);
	}
...
void Draw_WorldGeometry()
{
  ...
  for( int i=0; i<m_num_KWorldObjects ; i++ )
  {
	  ...
	  for( int j=0; j<m_KWorldObject[i].num_surfaces; j++ )
	  {
          ...
          glNormal3f( -1.0f*m_KWorldObject[i].surfaces[j].normal.x,
        		      -1.0f*m_KWorldObject[i].surfaces[j].normal.y,
        		      -1.0f*m_KWorldObject[i].surfaces[j].normal.z);
      ...
      else
      {
	   ...
	   if(m_KWorldObject[i].surface_type==cylinder)
	   {

         if(SimInfo.Simulation_Type==ballmill)
		 {
    	 float thickness=m_KWorldObject[i].cyl_geo.radius*1.025f -m_KWorldObject[i].cyl_geo.radius;
    	 ...
    	 /* Draw caps */
    	  glColor3f(0.2f,0.07f,0.06f);
    	    glPushMatrix();
    	      ...
    	      capb = gluNewQuadric();
    	      glTranslatef(m_KWorldObject[i].cyl_geo.center_bot_cap.x, m_KWorldObject[i].cyl_geo.center_bot_cap.y,
    	     		    	    		                                m_KWorldObject[i].cyl_geo.center_bot_cap.z );
			  if(SimInfo.Simulation_Type!=ballmill)
			{
    	     ...
    	     gluDisk(capb,m_KWorldObject[i].cyl_geo.radius, m_KWorldObject[i].cyl_geo.radius+thickness,120,120);
    	    glPopMatrix();
    	    ...
    	    glColor3f(0.2f,0.07f,0.06f);
      	    glPushMatrix();
      	      ...
      	      capt = gluNewQuadric();
      	      glTranslatef(m_KWorldObject[i].cyl_geo.center_top_cap.x, m_KWorldObject[i].cyl_geo.center_top_cap.y,
      	     		    	    		                                m_KWorldObject[i].cyl_geo.center_top_cap.z );
			  ...
			  if(SimInfo.Simulation_Type!=ballmill)
			{
			  glRotatef(-90.0f, 1.0f, 0.0f,0.0f);
      	      ...
      	      gluDisk(capt,m_KWorldObject[i].cyl_geo.radius, m_KWorldObject[i].cyl_geo.radius+thickness,120,120);
      	    glPopMatrix();
           /* End Draw Caps */
		 }


          /* 1. TO MAKE GEOMETRY HIDDEN COMMENT THIS OUT */
		  /*-------------------------------------------------*/
      	  glColor3f(0.2f,0.07f,0.06f);
    	  glPushMatrix();
    		...
    		gluCylinder( cyl,m_KWorldObject[i].cyl_geo.radius,
    				m_KWorldObject[i].cyl_geo.radius,
    				m_KWorldObject[i].cyl_geo.height,120,120 );
		 ...
		 if(SimInfo.Simulation_Type==ballmill)
		 {
    	  ...
    	  glPushMatrix();
			...
			gluCylinder( cylo,m_KWorldObject[i].cyl_geo.radius*1.025,
    				m_KWorldObject[i].cyl_geo.radius*1.025f,
    				m_KWorldObject[i].cyl_geo.height,80,80 );
    	  glPopMatrix();
	     }
	   }
	   else /* trunc cone */
     ...
     /* 2. TO MAKE GEOMETRY HIDDEN COMMENT THIS OUT */
     /*-------------------------------------------------*/
    	  ...
    	  glPushMatrix();
    		GLUquadricObj *cyl=NULL;
    		cyl = gluNewQuadric();
    	    glTranslatef(m_KWorldObject[i].cyl_geo.center_top_cap.x, m_KWorldObject[i].cyl_geo.center_bot_cap.y,
    	    		                                                 m_KWorldObject[i].cyl_geo.center_top_cap.z);
			...
			gluCylinder( cyl,m_KWorldObject[i].cyl_geo.radius_bot,
    				m_KWorldObject[i].cyl_geo.radius,
    				m_KWorldObject[i].cyl_geo.height,120,120 );
   ...
   /*-------------------------------------------------*/
		  ...
		  if(SimInfo.Simulation_Type==ballmill)
		  {
    		  glColor3f(0.f,0.0f,0.0f);
    		    ...
    		    glBegin(GL_LINES);
    		      glVertex3f(m_KWorldObject[i].cyl_geo.radius, -m_KWorldObject[i].cyl_geo.radius*0.025, m_KWorldObject[i].cyl_geo.height/2.0);
    		      glVertex3f(m_KWorldObject[i].cyl_geo.radius, m_KWorldObject[i].cyl_geo.radius*2.025, m_KWorldObject[i].cyl_geo.height/2.0);
    		    glEnd();
      		  ...
      		  glColor3f(0.f,0.0f,0.0f);
      		    glLineWidth((GLfloat)8);
      		    glBegin(GL_LINES);
      		      glVertex3f(-m_KWorldObject[i].cyl_geo.radius*0.025, m_KWorldObject[i].cyl_geo.radius, m_KWorldObject[i].cyl_geo.height/2.0);
      		      glVertex3f(m_KWorldObject[i].cyl_geo.radius*2.025, m_KWorldObject[i].cyl_geo.radius, m_KWorldObject[i].cyl_geo.height/2.0);
      		    glEnd();
		  }
		  else
		  {
              ...
              glColor3f(0.f,0.0f,0.0f);
    		    glLineWidth((GLfloat)8);
    		    glBegin(GL_LINES);
    		      glVertex3f(m_KWorldObject[i].cyl_geo.radius, m_KWorldObject[i].cyl_geo.height/2.0 , -m_KWorldObject[i].cyl_geo.radius*0.025);
    		      glVertex3f(m_KWorldObject[i].cyl_geo.radius, m_KWorldObject[i].cyl_geo.height/2.0, m_KWorldObject[i].cyl_geo.radius*2.025);
    		    glEnd();
      		  ...
      		  glColor3f(0.f,0.0f,0.0f);
      		    glLineWidth((GLfloat)8);
      		    glBegin(GL_LINES);
      		      glVertex3f(-m_KWorldObject[i].cyl_geo.radius*0.025, m_KWorldObject[i].cyl_geo.height/2.0, m_KWorldObject[i].cyl_geo.radius);
      		      glVertex3f(m_KWorldObject[i].cyl_geo.radius*2.025, m_KWorldObject[i].cyl_geo.height/2.0, m_KWorldObject[i].cyl_geo.radius);
      		    glEnd();
			  }
		  }
  ...
  for( int i=0; i<m_num_KVolumeObjects; i++ )
  {
     ...
     if(m_OpenGL.Debug_Lines==2)
     {
	  ...
	  glPushMatrix();
		GLUquadricObj *cyl=NULL;
		cyl = gluNewQuadric();
	    glTranslatef(m_KVolumeObject[i].COM.x, m_KVolumeObject[i].COM.y,
	    		m_KVolumeObject[i].COM.z);

	    if (m_KVolumeObject[i].Axis.x==0.0f)
	    {
	      glRotatef(90.0f,0.0f,1.0f,0.0f);
	    }
	    if (m_KVolumeObject[i].Axis.y==0.0f)
	    {
	      glRotatef(90.0f,0.0f,0.0f,1.0f);
	    }

		gluCylinder( cyl,m_KVolumeObject[i].boundR, m_KVolumeObject[i].boundR, 5.0,120,120 );
	  glPopMatrix();

     }

  	    for( int j=0; j< m_KVolumeObject[i].num_faces; j++ )
	    {

		     if(m_OpenGL.Debug_Lines==2)
		     {
  	    	   Draw_DebugLines( m_KVolumeObject[i].faces[j].centroid, m_KVolumeObject[i].faces[j].normal);
		     }

		   if( m_KVolumeObject[i].faces[j].num_vertex==3 )
		   {
			 glBegin(GL_LINE_LOOP);
		   }
		   else
		   {
		     ...
		     glMaterialf(GL_FRONT, GL_SHININESS, 5);
               ...
               glNormal3f( m_KVolumeObject[i].faces[j].normal.x,
        	 	           m_KVolumeObject[i].faces[j].normal.z,
        		           m_KVolumeObject[i].faces[j].normal.y);

		       for( int k=0; k<m_KVolumeObject[i].faces[j].num_vertex; k++ )
		       {

			      float3 vertex = m_KVolumeObject[i].vertex
			    		     [ m_KVolumeObject[i].faces[j].vertex_Order[k] ];
         ...
         if(SimInfo.isMillSim)
         {
             ...
             glNormal3f(0.0,0.0,1.0f);
		       ...
		       if(i==0)
		       {
			      ...
			      glVertex3f(m_KVolumeObject[i].vertex[0].x,m_KVolumeObject[i].vertex[0].y,m_KVolumeObject[i].vertex[0].z);
			      glVertex3f(m_KVolumeObject[i].vertex[3].x,m_KVolumeObject[i].vertex[3].y,m_KVolumeObject[i].vertex[3].z);

			      glVertex3f(m_KVolumeObject[i].vertex[7].x,m_KVolumeObject[i].vertex[7].y,m_KVolumeObject[i].vertex[7].z);


			      glVertex3f(m_KVolumeObject[i].vertex[4].x,m_KVolumeObject[i].vertex[4].y,m_KVolumeObject[i].vertex[4].z);
		  	    ...
		  	    /* Draw Dummy Ends */
					 ...
					 glNormal3f(0.0,0.0,1.0f);
				       ...
				       if(i==0)
				       {
					      ...
					      glVertex3f(m_KVolumeObject[i].vertex[2].x,m_KVolumeObject[i].vertex[2].y,m_KVolumeObject[i].vertex[2].z);
					      glVertex3f(m_KVolumeObject[i].vertex[1].x,m_KVolumeObject[i].vertex[1].y,m_KVolumeObject[i].vertex[1].z);

					      glVertex3f(m_KVolumeObject[i].vertex[5].x,m_KVolumeObject[i].vertex[5].y,m_KVolumeObject[i].vertex[5].z);
					      glVertex3f(m_KVolumeObject[i].vertex[6].x,m_KVolumeObject[i].vertex[6].y,m_KVolumeObject[i].vertex[6].z);
...
void keyboard(unsigned char key,int,int)
{
    ...
    switch (key)
    {
        ...
        case 'o':
        	...
        	if(!m_Silo_SimData.is_hatch_open)
        	{
			  ...
			  printf("here sim type %d \n",m_Sim_Type);
        	  if(m_Sim_Type==silo)
        	  {				 
				simtime_elapsed=0.0f;
				m_Current_StepNumber=0;
        	    printf("Manual Hatch opened at: %f (seconds) \n",simtime_elapsed );
   ...
   //         glReadPixels(0, 0, width, height, GL_RGB, GL_UNSIGNED_BYTE, pixels);
		...
		/* Control global rotation for mills and rotating world objects */
        case '+':
...
{
  case GLUT_KEY_UP:
  Translate_VolObject(0, 0.1f*make_float3(0.0f,1.0f,0.0f));
  break;	
  case GLUT_KEY_DOWN:
  Translate_VolObject(0,-0.1f*make_float3(0.0f,1.0f,0.0f));
  break;
  case GLUT_KEY_LEFT:
  Translate_VolObject(0, -0.1f*make_float3(1.0f,0.0f,0.0f));
  break;
  case GLUT_KEY_RIGHT:
  Translate_VolObject(0, 0.1f*make_float3(1.0f,0.0f,0.0f));
  break;
